function showProductModal(id){
  market.products(id, function(err, res){
    market.productToSeller(id, function(err1, res1){
      ...
      $("#productModal").find("#product_price").html(web3.fromWei(res[2].toNumber(), "ether"));
    ...
    market.buyProduct(
        ...
        function(err, succ){
          if(err == null){
            ...
            $("#orders").append(`<li class="list-group-item" id="loaderOrder"><img style="width: 100px; height: auto" src="assets/images/loader.gif"/></li>`);
          } else {
            console.log(err);
          }
        }
...
$(function () {  // equivalent to $(document).ready(...)
  if (typeof(web3) === "undefined") {
    ...
    const OrderCreatedEvt = market.OrderCreated({buyer: coinbase}, {
      fromBlock: 0
    });
    OrderCreatedEvt.watch(function(err, res){
      if(err){
        console.log(err);
        return;
      }
      console.log("Event: orderCreated");
      const ord = $("#loaderOrder");
      if(ord != null){
        ord.remove();
        if(!$("#order"+res.args._orderId).length){
          let id = res.args._orderId;
          market.getOrder(id, function(err, res){
            $("#orders").append(`<li class="list-group-item" id="order'+id+'">Order No. ${id}</li>`);
          });
        }
      }
    });
      ...
      for(let i = 0; i < res.toNumber(); i++){
        market.products(i, function(err, res){
          market.productToSeller(i, function(err1, res1){
            addProduct(i, res[0], res[1], web3.fromWei(res[2].toNumber(), "ether"), res[3].toNumber(), res1);
          })
        });
      }
    });
